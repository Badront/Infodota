 ...
 * 18:37
 */
public class AbilityDao extends GeneralDao<Ability> {
    ...
    private static final String CREATE_TABLE_QUERY = "( "
            + GeneralDao.COLUMN_ID + " integer not null, "
    ...
    public void saveOrUpdate(SQLiteDatabase database, Ability entity) {
        if (!TextUtils.isEmpty(entity.getWasId())) {
            ...
            database.delete(getTableName(), GeneralDao.COLUMN_ID + " in (" + query + ")", ids);
        }
    ...
    public Ability getById(SQLiteDatabase database, long id) {
        Cursor cursor = database.query(
                getTableName()
                , getAllColumns(), GeneralDao.COLUMN_ID + " = ? or " + COLUMN_IDLIST + " like ?"
    ...
    public List<Ability> getNotThisHeroEntities(SQLiteDatabase database, long heroId) {
        Cursor cursor = database.query(getTableName(), getAllColumns(), COLUMN_HERO_ID + "<>? and " + GeneralDao.COLUMN_ID + "<>5002", new String[]{String.valueOf(heroId)}, null, null, getDefaultOrderColumns());
        try {
            List<Ability> abilities = new ArrayList<Ability>();
            if (cursor.moveToFirst()) {
                do {
    ...
    public List<Ability> getEntitiesByList(SQLiteDatabase database, List<Long> inGameList) {
        ...
        Cursor cursor = database.query(getTableName(), getAllColumns(), GeneralDao.COLUMN_ID + " in (" + qAsStr + ") " + likeAsStr, whereArgs, null, null, getDefaultOrderColumns());
        try {
            List<Ability> abilities = new ArrayList<Ability>();
            if (cursor.moveToFirst()) {
                do {
